...
void TsynthPitchBend(byte channel, int bend)
{
  pitchBend.amplitude(bend * 0.5 * pitchBendRange * DIV12 * DIV8192); //)0.5 to give 1oct max - spread of mod is 2oct
}

void TsynthControlChange(byte channel, byte control, byte value)
{
...
void TsynthProgramChange(byte channel, byte program)
{
  state = PATCH;
  // patchNo = program + 1;
  recallPatch(patchNo);
...
void TsynthMIDIClockStop()
{
  MIDIClkSignal = false;
}

void TsynthMIDIClock()
{
...
//   if(synthParam){
//     // Get rotary encoder1 value
//     encRead = knobRight1.read()/2;
//     if ((encRead > encPrevious + 3) || (encRead < encPrevious - 3) )
//     {
//       switch (state)
//       {
//         case PARAMETER:
...
//       encPrevious = encRead;
//     }
//     else if ((encRead < encPrevious - 3) || (encRead > encPrevious + 3))
//     {
//       switch (state)
//       {
//         case PARAMETER:
...
void TsynthNoteOn(byte channel, byte note, byte velocity)
{
...
void TsynthNoteOff(byte channel, byte note, byte velocity)
{
  if (unison == 0)
  {
...
void Tsynth_setup()
{
  ...
  MIDI.setHandlePitchBend(TsynthPitchBend);
  MIDI.setHandleControlChange(TsynthControlChange);
  MIDI.setHandleProgramChange(TsynthProgramChange);
  ...
  volumePrevious = 0;  // RE_READ; //Force volume control to be read and set to current

  recallPatch(patch_num+1);
